 ...
 */
class Game {
    /** An ArrayList that SAVES all the possible moves done so
     * that the undo method may be implemented easier. */
    private ArrayList<Game> saves;
    /** GameState that is archived in SAVES to be used in the
     * undo method. */
    ...
    /** The Undo method that reverts the game state back to the state
     * before the last move that was played. */
    void undo() {
        if (saves.size() > 0) {
            int last = saves.size() - 1;
            Game temp = new Game();
            temp.copyFrom(saves.get(last));
            this.copyFrom(temp);
            saves.remove(last);
        }
    }

    /** Initializes a variable, GAMESAVE as a copy of the current game
     * state then adds that game state to SAVES, the ArrayList that
     * holds all the moves ever made in a single game.*/
    void archiveState() {
        gameSave = new Game();
        gameSave.copyFrom(this);
        saves.add(gameSave);
    }
